    ...
    if (strcmp(req->method, "HEAD") != 0) {
        ...
        for (;;) {
            ...
            if (fwrite (buf, n, 1, out) < n) {
                log_exit("failed to write to socket %s: %s", info->path, strerror(errno));
            }
            close(fd);
        }
        fflush(out);
...
output_common_header_fields(struct HTTPRequest *req, FILE *out, char *status)
{
    ...
    t = time(NULL);
    tm = gmtime(&t);
    if (!tm) log_exit("gmtime() field: %s", strerror(errno));
...
method_not_allowed(struct HTTPRequest *req, FILE *out)
{
    ...
    fprintf(out, "</header>\r\n");
    fprintf(out, "<body>\r\n");
    fprintf(out, "<p>The request method %s is not allowd</p>\r\n", req->method);
    fprintf(out, "</body>\r\n");
    fprintf(out, "</html>\r\n");
    fflush(out);
}
